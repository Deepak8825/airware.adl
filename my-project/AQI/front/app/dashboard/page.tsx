...
// Or create the file at src/components/ui/card.tsx (or .js) and export the required components.
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Sun, Cloud, CloudRain, CloudDrizzle, Wind, LogOut } from "lucide-react";
...
const Dashboard = () => {
  ...
  if (bootstrapping || isLoading || !aqiData) {
    return (
      ...
      <header className="bg-white/80 backdrop-blur-sm shadow-sm">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <Link href="/dashboard" className="text-2xl font-bold text-gray-800 hover:text-blue-600">{t.home.title}</Link>
          </div>
          <nav className="flex items-center space-x-6">
            <Link href="/dashboard" className="text-gray-600 hover:text-blue-600 font-medium">{t.nav.home}</Link>
            ...
            <div className="flex items-center gap-3">
              {profile && (
                <span className="text-sm text-gray-500 hidden sm:block">
                  {profile.name ? `Hi, ${profile.name.split(' ')[0]}` : profile.email}
                </span>
              )}
      ...
      <section className="container mx-auto px-4 py-12 text-center relative overflow-hidden">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-center gap-6 mb-8">
            <Link href="/dashboard" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-lg shadow-lg transition-transform transform hover:-translate-y-1 hover:scale-105 focus:ring-4 focus:ring-blue-300">{t.nav.goToDashboard}</Link>
